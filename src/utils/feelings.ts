...
export function randomFeeling(
    category: keyof typeof Feelings,
    group: keyof FeelingsData
): string {
    const feelingCategory = Feelings[category] as FeelingsData;
    const selectedGroup = feelingCategory[group];

    if (!selectedGroup || selectedGroup.length === 0) {
        throw new Error(`No data found for group: ${group} in category: ${category}`);
    }

    return getRandomItem(selectedGroup);
}